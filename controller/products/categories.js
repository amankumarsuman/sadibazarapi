...
export const getCategories = async (req, res) => {
  categories
    .find()
    .select("category subCategory cateimg id _id ")
    .exec()
    .then((docs) => {
      const response = {
        count: docs.length,
...
export const productsSearch = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ total_pages: numberOfPages, products: productsPaged });
  } catch (error) {
...
export const updateQuantity = async (req, res) => {
  try {
    ...
    for (const product of products) {
      const searchedProduct = await Products.findOne({
      ...
      if (searchedProduct.stock - product.quantity <= 0) {
        ...
        await Products.findOneAndUpdate(
          { product_id: product.product_id },
          { stock: searchedProduct.stock - product.quantity }
        );
      }
    }
...
export const ProductsRecommendations = async (req, res) => {
  ...
  try {
    ...
    products.push({
      ...
      products: productscategory1,
    });
    products.push({
      category: categories[1].category,
      products: productscategory2,
    });
...
const updateProducts = async (products) => {
  ...
  for (const product of products) {
    ...
    if (res.modifiedCount !== 0) updated.push(product);
